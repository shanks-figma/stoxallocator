...
import { Trash2, Plus, Minus, IndianRupee, RefreshCw, Shield, CheckCircle, XCircle, AlertCircle, TrendingUp, Wallet, BarChart2, ChevronRight } from "lucide-react";
    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-slate-950 border-b border-slate-800">
      <div className="mx-auto max-w-7xl px-6 h-16 flex items-center justify-between">
        ...
        <Link to="/" className="flex items-center gap-2.5">
          ...
          <span className="text-white font-bold text-lg tracking-tight">Stox<span className="text-emerald-400">Allocator</span></span>
        </Link>

        {/* Nav links */}
        <nav className="flex items-center gap-1">
          <Link to="/">
            <button className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${isActive("/") ? "bg-slate-800 text-white" : "text-slate-400 hover:text-white hover:bg-slate-800/50"}`}>
              <Wallet className="h-4 w-4" /> Allocator
            </button>
          </Link>
          <Link to="/order">
            <button className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${isActive("/order") ? "bg-slate-800 text-white" : "text-slate-400 hover:text-white hover:bg-slate-800/50"}`}>
              <ChevronRight className="h-4 w-4" /> Order (Exp)
            </button>
          </Link>
          <Link to="/buckets">
            <button className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${isActive("/buckets") ? "bg-slate-800 text-white" : "text-slate-400 hover:text-white hover:bg-slate-800/50"}`}>
              <BarChart2 className="h-4 w-4" /> Buckets
            </button>
          </Link>
          <Link to="/admin">
            <button className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${isActive("/admin") ? "bg-slate-800 text-white" : "text-slate-400 hover:text-white hover:bg-slate-800/50"}`}>
  ...
  return (
    <div className="min-h-screen bg-slate-950 pt-16">
      <div className="mx-auto max-w-7xl px-6 py-8">
        ...
        {/* Page header */}
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-white">Allocation Calculator</h1>
          <p className="text-slate-400 text-sm mt-1">Build your portfolio and estimate total investment cost</p>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-6">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full h-11 pl-10 pr-4 bg-slate-800 border border-slate-700 rounded-lg text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
                  />
                </div>
                <select
                  value={exchange}
                  onChange={(e) => setExchange(e.target.value)}
                  className="h-11 w-28 px-3 bg-slate-800 border border-slate-700 rounded-lg text-sm text-white focus:outline-none focus:ring-2 focus:ring-emerald-500"
                >
                ...
                <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {results.map((r) => (
                    <div
                      ...
                      className="flex items-center justify-between p-3 bg-slate-800 hover:bg-slate-700 border border-slate-700 hover:border-emerald-500/40 rounded-lg cursor-pointer transition-all group"
                    >
                      <div>
                        <div className="text-sm font-semibold text-white">{r.tradingsymbol || r.name}</div>
            ...
            <div className="bg-slate-900 rounded-xl border border-slate-800 overflow-hidden">
              <div className="flex items-center justify-between px-5 py-4 border-b border-slate-800">
                <div className="flex items-center gap-2">
                  <h2 className="text-sm font-semibold text-white">Stock Portfolio</h2>
                  {portfolio.length > 0 && (
              ...
              {portfolio.length === 0 ? (
                <div className="py-16 text-center">
                  ...
                  <div className="text-slate-400 text-sm font-medium">No stocks added yet</div>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-slate-800 bg-slate-900/50">
                        <th className="text-left text-xs text-slate-500 font-medium px-5 py-3">Stock</th>
                        <th className="text-center text-xs text-slate-500 font-medium px-3 py-3">Qty</th>
                        <th className="text-right text-xs text-slate-500 font-medium px-3 py-3">LTP</th>
                        <th className="text-right text-xs text-slate-500 font-medium px-3 py-3">Value</th>
                        <th className="text-right text-xs text-slate-500 font-medium px-3 py-3">
                          <TooltipProvider>
                            <Tooltip>
                        ...
                        <th className="text-right text-xs text-slate-500 font-medium px-5 py-3">Total</th>
                        <th className="px-3 py-3"></th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-800/60">
                      {portfolioWithLTP.map((row) => (
                        <tr key={row.instrument_key} className="hover:bg-slate-800/30 transition-colors">
                          <td className="px-5 py-3.5">
                            <div className="font-semibold text-white">{row.tradingsymbol || row.name}</div>
                          ...
                          <td className="px-3 py-3.5 text-right text-slate-300 font-mono text-xs">
          ...
          <div className="lg:w-2/5">
            <div className="sticky top-24 space-y-4">
              ...
              {/* Budget input */}
              <div className="bg-slate-900 rounded-xl border border-slate-800 p-5">
                <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">Total Budget</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full h-14 pl-9 pr-4 bg-slate-800 border border-slate-700 rounded-lg text-2xl font-bold text-white focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
                  />
                </div>
                <div className="text-center mt-3">
                  <div className="text-3xl font-bold text-white">{formatINR(Number(budget) || 0)}</div>
                  <div className="text-xs text-slate-500 mt-0.5">Available for investment</div>
                </div>
              </div>
              ...
              <div className="bg-slate-900 rounded-xl border border-slate-800 divide-y divide-slate-800">
                <div className="flex justify-between items-center px-5 py-4">
                  ...
                  <span className="font-semibold text-white font-mono">{formatINR(Number(budget) || 0)}</span>
                </div>
                <div className="flex justify-between items-center px-5 py-4">
                  <span className="text-sm text-slate-400">Invested (incl. charges)</span>
                  <span className="font-semibold text-white font-mono">{formatINR(finalCostTotal)}</span>
                </div>
                <div className="flex justify-between items-center px-5 py-4">
              ...
              <div className="bg-slate-900 rounded-xl border border-slate-800 p-5">
                <div className="flex justify-between items-center mb-3">
                  <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Investment Progress</span>
  ...
  return (
    <div className="min-h-screen bg-slate-950 pt-16">
      <div className="mx-auto max-w-3xl px-6 py-8">
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-white flex items-center gap-2"><Shield className="h-6 w-6 text-emerald-400" /> Admin Dashboard</h1>
          <p className="text-slate-400 text-sm mt-1">Manage Upstox API tokens and system configuration</p>
        </div>

        <div className="space-y-4">
          <div className="bg-slate-900 rounded-xl border border-slate-800 p-5">
            <h2 className="text-sm font-semibold text-white mb-4 flex items-center gap-2"><StatusIndicator status={status} /> System Status</h2>
            <div className="grid grid-cols-3 gap-4">
              <div className="bg-slate-800 rounded-lg p-3"><div className="text-xs text-slate-500 mb-1">Backend</div><div className="text-sm font-semibold text-emerald-400">Connected</div></div>
              <div className="bg-slate-800 rounded-lg p-3"><div className="text-xs text-slate-500 mb-1">Token</div><div className={`text-sm font-semibold ${status?.configured && status?.valid ? "text-emerald-400" : status?.configured ? "text-red-400" : "text-slate-500"}`}>{status?.configured ? (status?.valid ? "Valid" : "Expired") : "Not set"}</div></div>
              <div className="bg-slate-800 rounded-lg p-3"><div className="text-xs text-slate-500 mb-1">Last Check</div><div className="text-sm font-semibold text-slate-300">{status ? new Date().toLocaleTimeString() : "Never"}</div></div>
            </div>
          ...
          {schedulerStatus && (
            <div className={`rounded-xl border p-5 ${schedulerStatus.needs_refresh ? "bg-gradient-to-r from-red-900/30 to-orange-900/30 border-red-500/20" : "bg-gradient-to-r from-teal-900/30 to-cyan-900/30 border-teal-500/20"}`}>
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div><div className="text-xs text-slate-500 mb-1">Scheduler Status</div><div className={`text-sm font-semibold ${schedulerStatus.scheduler_running ? "text-emerald-400" : "text-amber-400"}`}>{schedulerStatus.scheduler_running ? "Running" : "Stopped"}</div></div>
                <div><div className="text-xs text-slate-500 mb-1">Last Refresh</div><div className="text-sm font-semibold text-slate-300">{schedulerStatus.last_refresh ? new Date(schedulerStatus.last_refresh).toLocaleString() : "Never"}</div></div>
                <div><div className="text-xs text-slate-500 mb-1">Next Refresh</div><div className="text-sm font-semibold" style={{color: schedulerStatus.needs_refresh ? "#f87171" : "#14b8a6"}}>{schedulerStatus.next_refresh ? new Date(schedulerStatus.next_refresh).toLocaleString() : "Not scheduled"}</div></div>
                <div><div className="text-xs text-slate-500 mb-1">Status</div><div className={`text-sm font-semibold ${schedulerStatus.refresh_status === "valid" ? "text-emerald-400" : schedulerStatus.refresh_status === "expired" || schedulerStatus.refresh_status === "token_missing" ? "text-red-400" : schedulerStatus.refresh_status === "failed" ? "text-red-400" : "text-slate-400"}`}>{schedulerStatus.refresh_status}</div></div>
              </div>
          ...
          <div className="bg-slate-900 rounded-xl border border-slate-800 p-5">
            <h2 className="text-sm font-semibold text-white mb-4">Admin Authentication</h2>
            <label className="block text-xs text-slate-400 mb-2">Admin Key</label>
            <input type="password" placeholder="Enter admin key from backend/.env" value={adminKey} onChange={(e) => setAdminKey(e.target.value)} className="w-full h-10 px-3 bg-slate-800 border border-slate-700 rounded-lg text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 mb-3" />
            <div className="flex gap-2">
              <button onClick={checkStatus} disabled={loading || !adminKey} className="px-4 py-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-sm text-white rounded-lg disabled:opacity-40 transition-colors">{loading ? "Checking…" : "Check Status"}</button>
              <button onClick={testConnection} disabled={loading} className="px-4 py-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-sm text-white rounded-lg disabled:opacity-40 transition-colors">Test Connection</button>
            </div>
          </div>
          ...
          <div className="bg-slate-900 rounded-xl border border-slate-800 p-5">
            <h2 className="text-sm font-semibold text-white mb-1">Manual Token (Fallback)</h2>
    ...
    <div className="min-h-screen bg-slate-950 pt-16">
      <div className="mx-auto max-w-5xl px-6 py-8">
        <div className="mb-8">
          ...
          <p className="text-slate-400 text-sm mt-1">Group stocks into portfolios and track performance</p>
        </div>

        <div className="bg-slate-900 rounded-xl border border-slate-800 p-5 mb-6">
          <h2 className="text-sm font-semibold text-white mb-4">Create New Bucket</h2>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-4">
            <input placeholder="Bucket name" value={name} onChange={(e) => setName(e.target.value)} className="h-10 px-3 bg-slate-800 border border-slate-700 rounded-lg text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500" />
            <select value={type} onChange={(e) => setType(e.target.value)} className="h-10 px-3 bg-slate-800 border border-slate-700 rounded-lg text-sm text-white focus:outline-none focus:ring-2 focus:ring-emerald-500">
              <option value="watchlist">Watchlist</option>
              <option value="allocation">Allocation</option>
            </select>
            <select value={progressMode} onChange={(e) => setProgressMode(e.target.value)} className="h-10 px-3 bg-slate-800 border border-slate-700 rounded-lg text-sm text-white focus:outline-none focus:ring-2 focus:ring-emerald-500">
        ...
        <div className="bg-slate-900 rounded-xl border border-slate-800 overflow-hidden">
          <div className="px-5 py-4 border-b border-slate-800">
            <h2 className="text-sm font-semibold text-white">Your Buckets</h2>
          </div>
          {(!buckets || buckets.length === 0) ? (
            <div className="py-16 text-center">
              <div className="h-12 w-12 rounded-full bg-slate-800 flex items-center justify-center mx-auto mb-3"><BarChart2 className="h-6 w-6 text-slate-600" /></div>
              <div className="text-slate-400 text-sm">No buckets yet. Create one above.</div>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-5">
              {buckets.map((b) => (
                <div key={b.id} className="bg-slate-800 rounded-xl border border-slate-700 p-4 hover:border-emerald-500/30 transition-colors">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                      {renamingId === b.id ? (
                        ...
                        <div onClick={() => { setRenamingId(b.id); setNewName(b.name); }} className="font-semibold text-white cursor-pointer hover:text-emerald-400 transition-colors">
                          {b.name}
                        </div>
                      )}
                      <div className="text-xs text-slate-500 mt-0.5">{b.type} · {b.progress_mode}</div>
                    </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="bg-slate-900 rounded-xl border border-slate-800 p-4">
                ...
                <div className="text-xs text-slate-500 mt-0.5">{bucket.type} · {bucket.progress_mode}</div>
              </div>
              <div className="bg-slate-900 rounded-xl border border-slate-800 p-4">
                <div className="text-xs text-slate-500 mb-1">Benchmark Value</div>
                <div className="font-bold text-white text-lg font-mono">{formatINR(summary.benchmark)}</div>
                <div className="text-xs text-slate-500 mt-0.5">Current: {formatINR(summary.current)}</div>
              </div>
              <div className="bg-slate-900 rounded-xl border border-slate-800 p-4">
                <div className="text-xs text-slate-500 mb-1">Overall Change</div>
                ...
                <div className="text-xs text-slate-500 mt-0.5">{formatINR(summary.current - summary.benchmark)}</div>
              </div>
            </div>

            {/* Actions */}
            <div className="bg-slate-900 rounded-xl border border-slate-800 p-5">
              <h2 className="text-sm font-semibold text-white mb-4">Baseline & Actions</h2>
              <div className="flex flex-wrap items-center gap-3">
                <input placeholder="Snapshot label (optional)" id="baseline-label" className="h-9 px-3 bg-slate-800 border border-slate-700 rounded-lg text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 w-48" />
                <button onClick={() => { const label = document.getElementById("baseline-label").value; setBaseline(label); }} className="h-9 px-4 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-sm text-white rounded-lg transition-colors">Set Baseline</button>
                <button onClick={loadMetrics} className="h-9 px-4 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-sm text-white rounded-lg transition-colors flex items-center gap-1.5"><RefreshCw className="h-3.5 w-3.5" /> Refresh</button>
                <select className="h-9 px-3 bg-slate-800 border border-slate-700 rounded-lg text-sm text-white focus:outline-none focus:ring-2 focus:ring-emerald-500" value={activeBaseline?.at || ""} onChange={(e) => selectBaseline(e.target.value)}>
            ...
            {/* Add stocks */}
            <div className="bg-slate-900 rounded-xl border border-slate-800 p-5">
              <h2 className="text-sm font-semibold text-white mb-4">Add Stocks</h2>
              <div className="flex gap-3">
                <div className="relative flex-1">
                  ...
                  <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search by name or symbol…" className="w-full h-10 pl-10 pr-4 bg-slate-800 border border-slate-700 rounded-lg text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500" />
                </div>
                <select value={exchange} onChange={(e) => setExchange(e.target.value)} className="h-10 px-3 bg-slate-800 border border-slate-700 rounded-lg text-sm text-white focus:outline-none focus:ring-2 focus:ring-emerald-500">
                  <option value="NSE">NSE</option>
                  <option value="BSE">BSE</option>
                </select>
              </div>
              {results.length > 0 && (
                <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {results.map((r) => (
                    <div key={r.instrument_key} onClick={() => addItem(r)} className="flex items-center justify-between p-3 bg-slate-800 hover:bg-slate-700 border border-slate-700 hover:border-emerald-500/40 rounded-lg cursor-pointer transition-all group">
                      <div><div className="text-sm font-semibold text-white">{r.tradingsymbol || r.name}</div><div className="text-xs text-slate-400">{r.name}</div></div>
            ...
            <div className="bg-slate-900 rounded-xl border border-slate-800 overflow-hidden">
              ...
              {sortedRows.length === 0 ? (
                <div className="py-16 text-center"><div className="text-slate-500 text-sm">No stocks added yet.</div></div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead><tr className="border-b border-slate-800 bg-slate-900/50">
                      <th className="text-left text-xs text-slate-500 font-medium px-5 py-3">Stock</th>
                      {bucket.type === "allocation" && <th className="text-center text-xs text-slate-500 font-medium px-3 py-3">Qty</th>}
                      <th className="text-right text-xs text-slate-500 font-medium px-3 py-3">Benchmark</th>
                      <th className="text-right text-xs text-slate-500 font-medium px-3 py-3">Current</th>
                      <th className="text-right text-xs text-slate-500 font-medium px-3 py-3">Change</th>
                      <th className="px-3 py-3"></th>
                    </tr></thead>
                    <tbody className="divide-y divide-slate-800/60">
                      {sortedRows.map((r) => (
                        <tr key={r.instrument_key} className="hover:bg-slate-800/30 transition-colors">
                          <td className="px-5 py-3.5"><div className="font-semibold text-white">{r.tradingsymbol || r.name}</div><div className="text-xs text-slate-500">{r.name}</div></td>
                          ...
                          <td className="px-3 py-3.5 text-right text-slate-300 font-mono text-xs">{r.ltp ? formatINR(r.ltp) : "—"}</td>
  ...
  return (
    <div className="min-h-screen bg-slate-950 pt-16">
      <div className="mx-auto max-w-7xl px-6 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-bold text-white">Batch Order Executor</h1>
          ...
          <p className="text-slate-400 text-sm mt-1">Build portfolio & execute all orders at once</p>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="flex-1 lg:w-3/5 space-y-4">
              ...
              {results.length > 0 && (
                <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {results.map((r) => (
                    ...
                    >
                      <div>
                        <div className="text-sm font-semibold text-white">{r.tradingsymbol}</div>
                        <div className="text-xs text-slate-400 truncate max-w-[160px]">{r.name}</div>
                      </div>
                      <div className="h-7 w-7 rounded-full bg-slate-700 group-hover:bg-emerald-500 flex items-center justify-center transition-colors">
                        <Plus className="h-3.5 w-3.5 text-slate-400 group-hover:text-white" />
            ...
            <div className="bg-slate-900 rounded-xl border border-slate-800 overflow-hidden">
              <div className="flex items-center justify-between px-5 py-4 border-b border-slate-800">
                <div className="flex items-center gap-2">
                  <h2 className="text-sm font-semibold text-white">Order Portfolio</h2>
                  {portfolio.length > 0 && (
                    <span className="text-xs bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 px-2 py-0.5 rounded-full">{portfolio.length} stocks</span>
                  )}
                </div>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-slate-800 bg-slate-900/50">
                        ...
                        <th className="text-right text-xs text-slate-500 font-medium px-3 py-3">LTP</th>
                        <th className="text-right text-xs text-slate-500 font-medium px-3 py-3">Value</th>
                        <th className="text-right text-xs text-slate-500 font-medium px-3 py-3">Charges</th>
                        <th className="text-right text-xs text-slate-500 font-medium px-3 py-3">Total</th>
                        <th className="px-3 py-3"></th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-800/60">
                      {portfolioWithLTP.map((row) => (
                        <tr key={row.instrument_key} className="hover:bg-slate-800/30 transition-colors">
                          <td className="px-5 py-3.5">
                            <div className="font-semibold text-white">{row.tradingsymbol}</div>
                            <div className="text-xs text-slate-500 truncate max-w-[160px]">{row.name}</div>
                          </td>
                          <td className="px-3 py-3.5 text-center">
          ...
          <div className="lg:w-2/5">
            <div className="sticky top-24 space-y-4">
              ...
              <div className="bg-slate-900 rounded-xl border border-slate-800 p-5">
                ...
                <div className="text-center mt-3">
                  <div className="text-3xl font-bold text-white">{formatINR(Number(budget) || 0)}</div>
                  <div className="text-xs text-slate-500 mt-0.5">Available budget</div>
                </div>
              </div>
              ...
              <div className="bg-slate-900 rounded-xl border border-slate-800 divide-y divide-slate-800">
                <div className="flex justify-between items-center px-5 py-4">
                  ...
                  <span className="font-semibold text-white font-mono">{formatINR(portfolioValue)}</span>
                </div>
                <button
                ...
                {showChargesBreakdown && portfolioValue > 0 && (
                  <div className="bg-slate-800/50 px-5 py-4 space-y-2 text-xs">
                    {(() => {
                      ...
                      return (
                        <>
                          <div className="flex justify-between"><span className="text-slate-500">STT (0.1%)</span><span className="text-slate-300 font-mono">{formatINR(breakdown.stt)}</span></div>
                          <div className="flex justify-between"><span className="text-slate-500">Exchange Fee</span><span className="text-slate-300 font-mono">{formatINR(breakdown.exchange)}</span></div>
                          <div className="flex justify-between"><span className="text-slate-500">Stamp Duty (0.015%)</span><span className="text-slate-300 font-mono">{formatINR(breakdown.stamp)}</span></div>
                          <div className="flex justify-between"><span className="text-slate-500">SEBI Charges</span><span className="text-slate-300 font-mono">{formatINR(breakdown.sebi)}</span></div>
                          <div className="flex justify-between"><span className="text-slate-500">GST (18%)</span><span className="text-slate-300 font-mono">{formatINR(breakdown.gst)}</span></div>
                          <div className="flex justify-between"><span className="text-slate-500">API Brokerage ({portfolio.length} × ₹10)</span><span className="text-slate-300 font-mono">₹{(portfolio.length * 10).toFixed(0)}</span></div>
